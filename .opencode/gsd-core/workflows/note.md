...
- **Global scope**: `D:/good/src/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/good/src/.opencode/notes/`)
...
2. Glob `D:/good/src/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/good/src/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/good/src/.opencode/notes/` — works in any directory
